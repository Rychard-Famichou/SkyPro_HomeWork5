...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Подключение к PostgreSQL для создания базы '{target_db}'..."
        )
        ...
        try:
            with psycopg.connect(
                ...
                with conn.cursor() as cur:
                    ...
                    if not exists:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"База данных '{target_db}' успешно создана!"
                            )
                        )
                    else:
            ...
            self.stdout.write(
                self.style.ERROR(f"Не удалось создать базу данных. Ошибка: {e}")
            )
