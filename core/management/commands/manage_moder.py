    ...
    def handle(self, *args, **options):
        ...
        if remove_action:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f'Ошибка: Группа "{group_name}" не существует.')
                )
                return
        else:
            ...
            if created:
                self.stdout.write(
                    self.style.WARNING(f'Группа "{group_name}" была создана.')
                )
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f"Ошибка: Пользователь с email {email} не найден.")
            )
            return
        ...
        if remove_action:
            if moderator_group in user.groups.all():
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Пользователь {email} удален из группы "{group_name}".'
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f'Пользователь {email} не состоял в группе "{group_name}".'
                    )
                )
        else:
            if moderator_group in user.groups.all():
                self.stdout.write(
                    self.style.WARNING(
                        f'Пользователь {email} уже находится в группе "{group_name}".'
                    )
                )
            else:
